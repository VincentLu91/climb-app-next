...
export default async function Home() {
  ...
  return (
    <main className="landing-page">
      ...
      <section id="loop" className="loop-section"><div className="section-intro"><p className="eyebrow">THE COACHING LOOP</p><h2>Not a report.<br /><em>A better next go.</em></h2></div><div className="loop-grid">{loopSteps.map(([number, title, copy], index) => <div className="loop-step" key={title}><span className="step-number">{number}</span><div><p className="step-title">{title}{index < loopSteps.length - 1 && <span className="step-arrow">→</span>}</p><p>{copy}</p></div></div>)}</div></section>
