...
function buildAttempts(messages) {
  ...
  for (const item of messages) {
    ...
    const isVideo =
      item.attachment?.signedUrl && item.attachment.media_type === "video";
    ...
    if (isUser) {
      ...
      const isSupportingImage =
        item.attachment?.signedUrl && item.attachment.media_type === "image";
...
export default function ChatPanel({
  ...
  useEffect(() => {
      ...
      if (item?.message)
        setMessages((current) => [
          ...current,
          {
            ...
            coachingHelpful: item.coachingHelpful ?? null,
          },
        ]);
    };
    ...
    const onProgress = (event) => {
      if (event.detail) setProgressState(event.detail);
    };
    window.addEventListener("climbing-coach-message", onCoach);
    window.addEventListener("climbing-user-attachment", onAttachment);
    window.addEventListener("climbing-progress-update", onProgress);
    return () => {
      window.removeEventListener("climbing-coach-message", onCoach);
      window.removeEventListener("climbing-user-attachment", onAttachment);
      window.removeEventListener("climbing-progress-update", onProgress);
    };
  }, []);
  ...
  async function processMessage(chatMessages, userText) {
    try {
      ...
      setMessages([
        ...chatMessages,
        { id, message: agentText, sender: "ChatGPT", coachingHelpful: null },
      ]);
      posthog.capture(
        "text_coach_interaction",
        { session_id: coachingSessionId, coach_message_id: id },
        { send_instantly: true },
      );
    } catch (error) {
      ...
      setMessages([
        ...chatMessages,
        { id, message: fallback, sender: "ChatGPT", coachingHelpful: null },
      ]);
    } finally {
      setTyping(false);
    }
  ...
  async function saveCoachingFeedback(messageId, helpful) {
    ...
    if (error) alert("Failed to save feedback.");
      ...
      setMessages((current) =>
        current.map((item) =>
          item.id === messageId ? { ...item, coachingHelpful: helpful } : item,
        ),
      );
      posthog.capture(
        "coaching_feedback_given",
        { session_id: coachingSessionId, coach_message_id: messageId, helpful },
        { send_instantly: true },
      );
    }
  ...
  async function send() {
    ...
    await processMessage(next, text);
  }
    ...
    <section className="coaching-grid">
      <div className="coaching-main">
        <div className="loop-label">
          <span className="live-dot" /> THE ADAPTIVE LOOP{" "}
          <span className="loop-line" />
        </div>
        <div className="next-card">
          <div className="next-card-top">
            <span className="next-kicker">TRY THIS NEXT</span>
          ...
          {currentExperiment && (
            <>
              {progressState?.active_limiter && (
                <p>
                  Based on your current limiter: {progressState.active_limiter}
                </p>
              )}
              <div className="next-action">
                <span>Retry the problem</span>
                <span>↓</span>
              </div>
            </>
          )}
        ...
        {progressState?.progress_note && (
          <div className="progress-note">
            ...
            <div>
              ...
              <p>{progressState.progress_note}</p>
            </div>
          </div>
        )}
        ...
        <div className="attempts-list">
          {attempts.length === 0 && (
            <div className="empty-attempt">
              <strong>Your first attempt starts the loop.</strong>
              <span>
                Upload a clip below and your coach will call out one thing to
                try.
              </span>
            </div>
          )}
          {attempts.map((attempt, index) => (
            ...
            >
              ...
              <div className="attempt-card">
                ...
                {attempt.media.media_type === "video" ? (
                  <video
                    className="attempt-media"
                    src={attempt.media.signedUrl}
                    controls
                  />
                ) : (
                  <img
                    className="attempt-media"
                    src={attempt.media.signedUrl}
                    alt={attempt.note || `Climbing attempt ${attempt.attempt}`}
                  />
                )}
                {attempt.note &&
                  attempt.note !== `Attempt ${attempt.attempt}` && (
                    <p className="attempt-caption">{attempt.note}</p>
                  )}
                {(attempt.coach ||
                  index === 0 ||
                  (index === attempts.length - 1 && currentProgressNote)) && (
                  <div className="insight-grid">
                    {attempt.coach && (
                      <div>
                        <span className="insight-label">COACH NOTICED</span>
                        <p>{attempt.coach.message}</p>
                      </div>
                    )}
                    {(index === 0 ||
                      (index === attempts.length - 1 &&
                ...
                {attempt.coach && (
                  <div className="coach-feedback">
                    <div>
                      {index === attempts.length - 1 && currentExperiment && (
                        <>
                          <span className="insight-label">NEXT EXPERIMENT</span>
                          <strong>{currentExperiment}</strong>
                        </>
                      )}
                    </div>
                    <div className="feedback-buttons">
                      <button
                        type="button"
                        onClick={() =>
                          saveCoachingFeedback(attempt.coach.id, true)
                        }
                        disabled={savingFeedbackMessageId === attempt.coach.id}
                      >
                        {attempt.coach.coachingHelpful === true
                          ? "Helpful ✓"
                          : "Helpful"}
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          saveCoachingFeedback(attempt.coach.id, false)
                        }
                        disabled={savingFeedbackMessageId === attempt.coach.id}
                      >
                        {attempt.coach.coachingHelpful === false
                          ? "Not quite"
                          : "Feedback"}
                      </button>
                    </div>
      ...
      <aside className="coach-sidebar">
        <div className="sidebar-heading">
          <div>
            <span className="eyebrow">COACHING THREAD</span>
            <h2>Talk it through</h2>
            <CreditBalance userId={userId} />
            <p className="credit-note">
              Text coaching doesn&apos;t use credits.
            </p>
          </div>
          <span className="coach-pulse" />
        </div>
        <p className="sidebar-copy">
          Ask a question between attempts. Keep the loop moving.
        </p>
        <div className="thread-messages">
          {messages
            ...
            .map((item, index) => (
              <div
                className={`thread-message ${item.sender === "user" ? "is-user" : "is-coach"}`}
                key={item.id ?? index}
              >
                <span>{item.sender === "user" ? "YOU" : "COACH"}</span>
                {item.message && <p>{item.message}</p>}
                {item.attachment?.signedUrl &&
                  item.attachment.media_type === "image" && (
                    <img
                      className="thread-attachment"
                      src={item.attachment.signedUrl}
                      alt={item.message || "Route or wall context"}
                    />
                  )}
              </div>
            ))}
